...
export default function App() {
  ...
  // Engine stores fractions (0.044 = 4.4%). Also tolerate a server sending 4.4.
  const fmtPricePct = (raw, fallbackPct = 4.4) => {
    const n = Number(raw);
    if (!Number.isFinite(n) || n === 0) return `${Number(fallbackPct).toFixed(2)}%`;
    const pct = n <= 1 ? n * 100 : n; // 0.044 → 4.4; 4.4 → 4.4
          ...
          <>
            ...
            <View style={styles.card}>
              ...
              <Text style={styles.help}>
                {`TP is fixed at coin price +${fmtPricePct(settings?.take_profit_price_pct ?? settings?.take_profit_pct, 4.4)}. CoinDCX displays that as about +44% ROE @10x or +22% ROE @5x.`}
                {"\n"}
                {`SL is fixed at coin price −${fmtPricePct(settings?.stop_loss_price_pct ?? settings?.stop_loss_pct, 1.5)}, giving TP:SL ≈ 2.9:1.`}
                {"\n"}
        ...
        {activeTab === 'settings' && (
          <>
            ...
            <View style={styles.card}>
              ...
              <Row C={C} label="Take profit" value={settings?.tp_display || `Price +${fmtPricePct(settings?.take_profit_price_pct ?? settings?.take_profit_pct, 4.4)}`} />
              <Row C={C} label="Stop loss" value={settings?.sl_display || `Price −${fmtPricePct(settings?.stop_loss_price_pct ?? settings?.stop_loss_pct, 1.5)}`} />
